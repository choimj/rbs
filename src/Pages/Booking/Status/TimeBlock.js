...
const createTimeBlock = (openTime, closeTime, day, bookingList) => {
  ...
  for (let i = openTime; i <= closeTime - 1; i++) {
    ...
    if (bookElement.length > 0) {
      let bookStartTime = new Date(
        day + " " + bookElement[0].startHour + ":" + bookElement[0].startMinute
      );
      if (leftSpanStartTime >= bookStartTime) {
        tmp.push(
          <span>
            {bookElement[0].startHour + ":" + bookElement[0].startMinute}
          </span>
        );
      }
    }
  ...
  timeSpan.map((item, i) => {
    resultArray.push(<Root key={i}>{item}</Root>);
    return resultArray;
  });
